...
val unionDF = dfList2.reduce(_ unionByName _)
...
cleanDF.write.format("parquet").save("/FileStore/tables/datasets/parquet")
...
val df1 = cleanDF2.groupBy("Id").count().where($"count" > 1)
val df2 = cleanDF2.select("Id","Name","Authors","ISBN","RatingDistTotal","pagesNumber")

val joinDF = df1.join(df2,df1("Id") === df2("Id"), "inner").drop(df2("Id")).orderBy(df2("Id"))
